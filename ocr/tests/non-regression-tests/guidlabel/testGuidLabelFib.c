...
ocrGuid_t complete(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    in2 = *(u32*)depv[1].ptr;
    out = *(u32*)depv[2].ptr;
    ocrPrintf("Done with %"PRId32" (%"PRId32" + %"PRId32")\n", out, in1, in2);
...
ocrGuid_t fibEdt(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    ocrPrintf("Starting fibEdt(%"PRIu32")\n", n);
    ocrAssert(n >= 2);
    ...
    ocrPrintf("In fibEdt(%"PRId32") -- checking for required answers\n", n);
    ...
    if(ocrEventCreate(&evt0, OCR_EVENT_STICKY_T, GUID_PROP_IS_LABELED | GUID_PROP_CHECK | EVT_PROP_TAKES_ARG) == OCR_EGUIDEXISTS) {
        ...
        ocrPrintf("In fibEdt(%"PRId32"), reusing answer for %"PRId32"\n", n, n-1);
    } else {
        ...
        ocrPrintf("In fibEdt(%"PRIu32") -- created arg DB for %"PRId32" GUID "GUIDF"\n", n, n-1, GUIDA(dbArg));
        *((u32*)ptr) = n-1;
    ...
    ocrPrintf("In fibEdt(%"PRIu32") -- spawned complete EDT GUID "GUIDF"\n", n, GUIDA(comp));
    ocrAddDependence(depv[0].guid, comp, 2, DB_DEFAULT_MODE);

    ocrPrintf("Returning from fibEdt(%"PRIu32")\n", n);
    return NULL_GUID;
