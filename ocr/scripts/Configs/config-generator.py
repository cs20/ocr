...
parser.add_argument('--numa', dest='numa', default='none',
                   help='provide information about numa nature of X86 targets')
...
def GenerateComp(output, pdtype, threads, binding, numa, sysworker, schedtype):
...
def GenerateConfig(filehandle, guid, platform, target, threads, binding, numa, sysworker, alloc, alloctype, dbtype):
    ...
    if target=='X86':
        ...
        GenerateComp(filehandle, "HC", threads, binding, numa, sysworker, "COMMON")
    elif (target=='FSIM'):
        ...
        if (scheduler == 'LEGACY'):
            GenerateComp(filehandle, pdtype, threads, binding, numa, sysworker, "HC_COMM_DELEGATE")
        else:
            GenerateComp(filehandle, pdtype, threads, binding, numa, sysworker, "COMMON")
...
if platform=='X86' or platform=='FSIM':
    ...
    if os.path.isfile(outputfilename) and (not rmdest):
        ...
        with open(outputfilename, "w") as filehandle:
            GenerateConfig(filehandle, guid, platform, target, threads, binding, numa, sysworker, alloc, alloctype, dbtype)
else:
