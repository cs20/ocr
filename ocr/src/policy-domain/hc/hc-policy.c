...
static u8 createEventHelper(ocrPolicyDomain_t *self, ocrFatGuid_t *guid,
                        ocrEventTypes_t type, u32 properties, ocrParamList_t *userArg, ocrParamList_t * paramList,
                        ocrFatGuid_t currentEdt) {
    u8 retCode = ((ocrEventFactory_t*)(self->factories[self->eventFactoryIdx]))->instantiate(
        (ocrEventFactory_t*)(self->factories[self->eventFactoryIdx]), guid, type, properties, userArg, paramList);
    return retCode;
}

#ifdef REG_ASYNC_SGL
static u8 convertDepAddToSatisfy(ocrPolicyDomain_t *self, ocrFatGuid_t dbGuid,
                                 ocrFatGuid_t destGuid, u32 slot, ocrDbAccessMode_t mode,
#ifdef ENABLE_AMT_RESILIENCE
...
#ifdef ENABLE_AMT_RESILIENCE
        ocrGuid_t resilientEdtParent = PD_MSG_FIELD_I(resilientEdtParent);
#endif
...
#define PRESCRIPTION 0x10LL
        ...
        if(PD_MSG_FIELD_O(returnDetail) == 0) {
            ...
            ASSERT(db);
#ifdef ENABLE_AMT_RESILIENCE
            db->resilientEdtParent = resilientEdtParent;
#endif
            if(doNotAcquireDb) {
...
#undef PD_TYPE
    ...
    case PD_MSG_DB_ACQUIRE: {
        ...
        #define PD_TYPE PD_MSG_DB_ACQUIRE
            ...
            ASSERT(db->fctId == ((ocrDataBlockFactory_t*)(self->factories[self->datablockFactoryIdx]))->factoryId);
#ifdef ENABLE_AMT_RESILIENCE
            ocrGuid_t resilientEdtParent = PD_MSG_FIELD_IO(resilientEdtParent);
            if (!ocrGuidIsNull(resilientEdtParent)) {
                salRecordEdt(resilientEdtParent);
            }
            PD_MSG_FIELD_IO(resilientEdtParent) = db->resilientEdtParent;
#endif
            PD_MSG_FIELD_O(returnDetail) = ((ocrDataBlockFactory_t*)(self->factories[self->datablockFactoryIdx]))->fcts.acquire(
...
#define PD_TYPE PD_MSG_EVT_CREATE
        ...
        if (PD_MSG_FIELD_I(params) != NULL) {
            ...
            userArg = (ocrParamList_t *) PD_MSG_FIELD_I(params);
        }
#endif
#ifdef ENABLE_AMT_RESILIENCE
        ocrGuid_t resilientEdtParent = PD_MSG_FIELD_I(resilientEdtParent);
        ...
        evtParams.resilientLatch = PD_MSG_FIELD_I(resilientLatch);
        ...
        if (isBlocking == false) {
            u32 returnDetail = createEventHelper(
                self, &(PD_MSG_FIELD_IO(guid)),
                PD_MSG_FIELD_I(type), (PD_MSG_FIELD_I(properties) | GUID_PROP_TORECORD), userArg, paramList,
