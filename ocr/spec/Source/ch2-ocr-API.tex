...
\item \code{\#define FALSE 0};
\item \code{\#define NULL\_HINT}: A NULL pointer to an ocrHint\_t.
\item \code{\#define NULL\_GUID}: A reserved GUID value, used to indicate the absence of an OCR object.
This value is roughly analogous to a NULL pointer; however, it cannot be assumed to be equal to zero.
\item \code{\#define UNINITIALIZED\_GUID}: A reserved GUID value, used as a placeholder to indicate
that a value is uninitialized.
\item \code{\#define ERROR\_GUID}: A reserved GUID value, used to indicate that an error has occurred.
\end{itemize}
%%%%%
\section{OCR API conventions}
\index{OCR API conventions}
...
\footnote{\hyperlink{func_ocrAddDependence}{\code{ocrAddDependence()}} being the lone exception to this naming
...
Immediate errors are typically errors that result from incorrect arguments. Deferred errors are
...
command line. The arguments can be accessed using \hyperlink{func_ocrGetArgc}{\code{ocrGetArgc()}}
and \hyperlink{func_ocrGetArgv}{\code{ocrGetArgv()}}.
...
\hline
\mbox{\tt in} & \code{paramv} & Parameter vector will always be NULL.\\
\hline
...
\item
  void \hyperlink{func_ocrAbort}{\code{ocrAbort}}(\hyperlink{type_u8}{u8} errorCode)
    ...
    \end{DoxyCompactList}
\item
  \hyperlink{type_u64}{u64} \hyperlink{func_ocrGetArgc}{\code{ocrGetArgc}}(void $\ast$dbPtr)
    \begin{DoxyCompactList}
      \small\item \emph{Retrieves the traditional `argc' value from
        \hyperlink{func_mainEdt}{\code{mainEdt()}}'s input data block.}
    \end{DoxyCompactList}
\item
  char $\ast$ \hyperlink{func_ocrGetArgv}{\code{ocrGetArgv}}(void $\ast$dbPtr,
  \hyperlink{type_u64}{u64} index)
    ...
    \end{DoxyCompactList}
\item
  \hyperlink{type_u32}{u32} \hyperlink{func_ocrPrintf}{\code{ocrPrintf}}(const char $\ast$format, ...)
    \begin{DoxyCompactList}
...
program can successfully complete even if not all EDTs have executed.
\end{DoxyNote}
%
% ocrAbort
%
...
itself down and some implementations may choose to crash out to provide, for
example, a core dump for debugging. This call should only be used to indicate an abnormal termination.
...
\label{func_ocrGetArgv}

Returns the 0-based $index^{th}$ argument passed to the OCR program.
The value is extracted from the unique data block passed to \hyperlink{func_mainEdt}{\code{mainEdt}}.
...
\descr
See Section~\ref{func_ocrGetArgc} for details.
\begin{DoxyNote}{Note}
Attempting to extract an argument with \code{index} greater or equal to the value
returned by \hyperlink{func_ocrGetArgc}{\code{ocrGetArgc}} will result in undefined
behavior.
\end{DoxyNote}
%
% ocrPrintf
%
\hypertarget{func_ocrPrintf}{
  \index{Supporting functions@{Supporting function}!ocrPrintf@{ocrPrintf}}
  \subsection[{ocrPrintf}]{\setlength{\rightskip}{0pt plus 5cm}{\bf u32} ocrPrintf(
\begin{DoxyParamCaption}
\item[{const char $\ast$}]{fmt}
\item[{}]{...}
\end{DoxyParamCaption}
)}}
\label{func_ocrPrintf}
...
\begin{DoxyNote}{Note}
A conforming implementation may limit the number of characters in the
output string.
\end{DoxyNote}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% GUID MANAGEMENT API
...
\item
  \hyperlink{type_bool}{bool} \hyperlink{func_ocrGuidIsLt}
    ...
    \hyperlink{type_ocrGuid_t}{ocr\-Guid\-\_\-t} guid2)
    \begin{DoxyCompactList}
      \small\item \emph{Checks if guid1 is less than guid2 (useful to implement a total order on GUIDs)}
    \end{DoxyCompactList}
\end{DoxyCompactItemize}
...
the only way to ``share'' data between EDTs. Conceptually, data blocks
...
of the EDT (or the data block creation) and the corresponding \code{ocrDbRelease}
(or the end of the EDT).
\end{itemize}
...
\item
  \hyperlink{type_u8}{u8} \hyperlink{func_ocrDbRelease}
    ...
    \end{DoxyCompactList}
\item
  \hyperlink{type_u8}{u8} \hyperlink{func_ocrDbDowngradeRelease}
            {\code{ocrDbDowngradeRelease}}(\hyperlink{type_ocrGuid_t}{ocr\-Guid\-\_\-t} db)
    \begin{DoxyCompactList}
      \small\item \emph{Release updates made to a writable data block
        while retaining read-only access to the data block.}
    \end{DoxyCompactList}
\end{DoxyCompactItemize}
%
%  ocrDbCreate
...
\item[{{\bf u64}}]{len, }
\item[{{\bf u16}}]{flags, }
\item[{{\bf const ocr\-Hint\-\_\-t} $\ast$}]{hint, }
...
undefined \\
\hline
\mbox{\tt in}  & \code{len} & Non-zero size, in bytes, of the data block to create \\
\hline
...
0 if no immediate error was detected or the following error codes:
\begin{DoxyItemize}
\item OCR\_EPERM ({\bf deferred}): The data block was already destroyed
\item OCR\_EINVAL ({\bf deferred}): The GUID passed as argument does not refer to
  a valid data block
\end{DoxyItemize}
...
0 if no immediate error was detected or the following error codes:
\begin{DoxyItemize}
\item OCR\_EINVAL ({\bf immediate}): The GUID passed as argument does not refer to a
  valid data block
\item OCR\_EACCESS ({\bf immediate}): The calling EDT has not acquired the data block
...
%
% ocrDbDowngradeRelease
%
\hypertarget{func_ocrDbDowngradeRelease}{
  \index{Data block management@{Data-\/block management}!ocr\-Db\-Downgrade\-Release@{ocr\-Db\-Downgrade\-Release}}
  \subsection[{ocr\-Db\-Downgrade\-Release}]{\setlength{\rightskip}{0pt plus 5cm}{\bf u8} ocr\-Db\-Downgrade\-Release(
\begin{DoxyParamCaption}
\item[{{\bf ocr\-Guid\-\_\-t}}]{db}
\end{DoxyParamCaption}
)}}
\label{func_ocrDbDowngradeRelease}

This function allows for any writes made to a data block in the current EDT
to be \textit{released} (in terms of OCR's release consistency memory model)
while still allowing the data block to be read later in the current EDT.
In contrast, a call to \hyperlink{func_ocrDbRelease}{\code{ocrDbRelease()}}
indicates the end of both write and read access to the target data block
within the current EDT.

\begin{DoxyParams}[1]{Parameters}
\mbox{\tt in}  & \code{db} & GUID of the data block to downgrade/release\\
\hline
\end{DoxyParams}
...
0 if no immediate error was detected or the following error codes:
\begin{DoxyItemize}
\item OCR\_EINVAL ({\bf deferred}): The GUID passed as argument does not refer to a
  valid data block
\item OCR\_EACCESS ({\bf immediate}): The calling EDT has not acquired the data block
  and therefore cannot downgrade/release it
\end{DoxyItemize}

\descr
OCR's release-consistency memory model requires a \textit{release}
operation on a data block for synchronization, ensuring that writes to that data block
are visible to all EDTs ordered after the \textit{release} operation.
For a data block that is acquired in a writable mode by the current EDT,
this function performs a \textit{release} operation
(making all preceding writes visible in terms of the memory model);
however, the current EDT retains read-only access to the data block
by \textit{downgrading} the data block's access mode to a read-only
access mode (i.e., \code{DB\_MODE\_RO} or \code{DB\_MODE\_CONST}).
The user should assume that the mode for the downgraded data block
is \code{DB\_MODE\_RO} but conformant implementations may downgrade
certain data blocks to \code{DB\_MODE\_CONST}.
If the target data block is already in a read-only access mode,
then this function does nothing.

\begin{DoxyNote}{Note}
Unlike \hyperlink{func_ocrDbRelease}{\code{ocrDbRelease()}},
...
a call to \hyperlink{func_ocrDbRelease}{\code{ocrDbRelease()}},
...
\item \hypertarget{type_ocrLatchEventSlots}{\code{enum ocrLatchEventSlots\_t}} containing constants to
identify the two pre-slots of the latch event type:
  \begin{itemize}
    \item \code{OCR\_EVENT\_LATCH\_DECR\_SLOT} identifying the decrement slot of
      the latch event. The numeric equivalent of this value is $0$.
    \item \code{OCR\_EVENT\_LATCH\_INCR\_SLOT} identifying the increment slot of
      the latch event. The numeric equivalent of this value is $1$.
  \end{itemize}
\end{itemize}
  ...
  \hyperlink{type_u8}{u8} \hyperlink{func_ocrEventSatisfySlot}
    ...
    \begin{DoxyCompactList}
      \small\item \emph{Satisfy the specified pre-slot of an event and optionally
        pass a data block to the event. Note that, in the case of a latch event, the slot
        parameter is of type \hyperlink{type_ocrLatchEventSlots}{enum\ ocrLatchEventSlots\_t}}
    \end{DoxyCompactList}
\end{DoxyCompactItemize}
%
% ocrEventCreate
...
\item EVT\_PROP\_TAKES\_ARG: If this flag is {\bf not} set, the event will ignore any data block
  passed to it when it is satisfied. The satisfaction call will raise a deferred error if a data block
  is passed in the satisfy call. If the flag is set, the event can take a data block or NULL\_GUID when
  it is satisfied. In other words, EVT\_PROP\_TAKES\_ARG, if set, only tells the runtime that the event
  {\bf may} take a data block on satisfaction but does not force it to.
\end{DoxyItemize}\\
\hline
\end{DoxyParams}
...
0 if no immediate error was detected or the following error codes:
\begin{DoxyItemize}
\item OCR\_ENOMEM ({\bf deferred}): The runtime could not satisfy the event due to insufficient
  memory
\item OCR\_EINVAL ({\bf deferred}): \code{eventGuid} or \code{dataGuid} do not refer
  to valid event or data block GUIDs respectively
\item OCR\_EACCES ({\bf deferred}): a non NULL\_GUID was passed as \code{dataGuid} when the
...
\item \code{EDT\_PROP\_OEVT\_VALID} which specifies that the output event
  passed when creating an EDT is a valid event
...
\item
  \hyperlink{type_u8}{u8} \hyperlink{func_ocrEdtTemplateCreate}
            {\code{ocrEdtTemplateCreate}}(\hyperlink{type_ocrGuid_t}{ocr\-Guid\-\_\-t} $\ast$guid,
...
0 if no immediate error was detected or the following error codes:
\begin{DoxyItemize}
\item OCR\_ENOMEM ({\bf deferred}): The runtime could not allocate the template
\end{DoxyItemize}
...
0 if no immediate error was detected or the following error codes:
\begin{DoxyItemize}
\item OCR\_EINVAL ({\bf deferred}): The GUID passed as argument does not refer to a valid
  EDT template
\end{DoxyItemize}
...
\item[{{\bf u32}}]{paramc, }
\item[{{\bf const u64} $\ast$}]{paramv, }
\item[{{\bf u32}}]{depc, }
\item[{{\bf const ocr\-Guid\-\_\-t} $\ast$}]{depv, }
\item[{{\bf u16}}]{flags, }
\item[{{\bf const ocr\-Hint\-\_\-t} $\ast$}]{hint, }
...
set to NULL.\\
\hline
\mbox{\tt in}  & \code{depc} & Number of pre-slots for this EDT.\\
\hline
...
\item EDT\_PROP\_OEVT\_VALID: The output event's (last argument) value
  is a valid event to use as an output event
\end{DoxyItemize}\\
\hline
\mbox{\tt in}  & \code{hint} & Reserved for future use. Set
to NULL\_HINT.\\
\hline
\mbox{\tt in,out}  & \code{outputEvent} & The meaning of this field is controlled
by the value of the `flags' argument. If EDT\_PROP\_OEVT\_VALID is specified in
the flags, outputEvent must be non-NULL and point to the GUID of a valid
user created event. Note that if the event given as an argument is a latch event,
only the OCR\_EVENT\_LATCH\_DECR\_SLOT is satisfied.
If EDT\_PROP\_OEVT\_VALID is not specified in the flags:
if non-NULL on input, on successful creation,
contains the GUID of the event associated with the post-slot of the EDT; if NULL,
no output event will be returned. When the event is automatically created by the
runtime (if EDT\_PROP\_OEVT\_VALID is not specified), the event returned will be
automatically destroyed on satisfaction. It is therefore crucial to ensure that
all waiters on this event are set up properly \emph{before} the EDT transitions to the
\emph{runnable} state. If this behavior is undesirable, the EDT\_PROP\_OEVT\_VALID allows
you to override the event that will be used as the output event and the programmer can
provide a sticky event which will not be destroyed on satisfaction.
In all cases, the post-slot of the output event
(either specified by the user or created by the runtime) will be satisfied:
{\bf a)} for a \emph{finish} EDT, when the EDT
and all of its descendant EDTs (the closure of all EDTs created in this EDT and
its children) have completed execution or {\bf b)} for a non finish EDT, the post-slot of
this event will be satisfied when the EDT completes execution and will carry the
data block returned by the EDT.\\
\hline
\end{DoxyParams}
...
0 if no immediate error was detected or the following error codes:
\begin{DoxyItemize}
\item OCR\_ENOMEM ({\bf deferred}): The runtime could not create the EDT
\item OCR\_EINVAL ({\bf deferred}): The GUID specifying the template is invalid or the number
  of parameters or pre-slots are invalid or the combination of flags and other parameter values
  is invalid
\end{DoxyItemize}
...
pre-slots (\code{depc}) be resolved at this time. In other words, the number of
parameters and dependences must be explicitly specified at this time. If the
EDT template used for this EDT specified specific numbers of parameters or dependences,
the values for \code{paramc} and \code{depc} must match those specified for
the template.
\begin{DoxyNote}{Note}
If the EDT is created with all its pre-slots specified and resolved, it may execute
immediately, which means that the GUIDs returned (\code{guid} and \code{outputEvent}, if
created by the runtime)
may be invalid by the time this function returns. It is the responsibility of
the programmer -- if these returned values are needed -- to ensure that the EDT
cannot start until a later time (e.g., by inserting an extra pre-slot
that is satisfied only after setting up all dependences on the output event).
\end{DoxyNote}
%
% ocrEdtDestroy
%
...
\returns
0 if no immediate error was detected. There are no error codes for this function.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% OCR Dependences
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
0 if no immediate error was detected or the following error codes:
\begin{DoxyItemize}
\item OCR\_EINVAL ({\bf deferred}): \code{slot} is invalid
\item OCR\_ENOPERM ({\bf deferred}): \code{source} and \code{destination} cannot be linked with
...
\item Data block to event: Adding a dependence between a data block and an
  ...
  i.e., \code{ocrAddDependence(db, evt, slot, mode)} is
  similar to \hyperlink{func_ocrEventSatisfySlot}{
  \code{ocrEventSatisfySlot}}\code{(evt, db, slot)},
